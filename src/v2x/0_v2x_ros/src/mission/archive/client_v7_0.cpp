...
public:	
	...
	void initParams(void) {
		...
		if (yaml_mission_list.size() == 0){
			yaml_mission_list.emplace_back(11);
			yaml_mission_list.emplace_back(12);
			yaml_mission_list.emplace_back(13);
		}
	...
	void sendCurrentLocation(double a_latitude, double a_longitude, float a_elevation, unsigned short a_heading, unsigned char a_speed) {
		...
		unsigned char speed = a_speed;
			...
			if (sentisize  > -1) {
				...
				ROS_WARN("FAILED TO SEND LOCATION PACKET, PLEASE CHECK CONNECTION STATUS");
			}
	...
	void posealertCallback(const novatel_gps_msgs::InspvaPtr& gpsHandler){
		...
		if ((is_v2x_on_ == true) && (is_connect_ != -1) && (timediff > 0.1)){
			ROS_WARN("STARTING SEND CURRENT LOCATION THREAD!");
