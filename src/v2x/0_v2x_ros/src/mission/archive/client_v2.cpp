...
private:
	...
	ros::Publisher  pickup_pub_, dropoff_pub_, target_pub_;
...
public:	
		...
		private_nh_("~")	
	{
		...
		if (v2x_operation_env == "OFFLINE"){
			//////////////////////////////// 	V2X OFFLINE TESTING		////////////////////////////
			curr_pos_lat = 35.880143;
			curr_pos_long = 128.626841;
			curr_pos_z = 100;
			...
			ROS_ERROR("v2x_operation_env string is |%s|",v2x_operation_env.c_str()); 
			ros::shutdown();
		}
		...
		if (custom_mission_list[0] < 10000){
			...
			std::cout<<"custom mission given in UTM coordinates"<<std::endl;
	...
	bool sendCurrentLocation(double latitude, double longitude, float elevation, unsigned short heading, unsigned char speed ) {
	...
	void posealertCallback(const novatel_gps_msgs::InspvaPtr& gpsHandler){
		...
		if (v2x_operation_env == "ONLINE"){
			sendCurrentLocation(send_lat, send_long, send_elev, send_heading, send_speed);
		...
		// for time check
		prev_odom_time = ros::Time::now().toSec();
		usleep(100); //100 ms 주기로 메시지가 보내져야함 (50ms ~ 200ms 사이)
	}
	...
	void checkPriority(void) {
		...
		geometry_msgs::PoseStamped pickup_rviz, dropoff_rviz;
		pickup_rviz.pose.position.x = cur_mission_.start_xpos;
		pickup_rviz.pose.position.y = cur_mission_.start_ypos;
		pickup_pub_.publish(pickup_rviz);
		dropoff_rviz.pose.position.x = cur_mission_.goal_xpos;
		dropoff_rviz.pose.position.y = cur_mission_.goal_ypos;
		dropoff_pub_.publish(dropoff_rviz);
	}

	void checkMissionState(void) {
	
		geometry_msgs::PoseStamped target_msg; // 현재의 Global Planning 목표
		
		double x = curr_pos_lat;
		double y = curr_pos_long;
		float v = curr_speed;

		double dist2start = lindist(cur_mission_.start_xpos, cur_mission_.start_ypos, x, y );
		double dist2goal = lindist(cur_mission_.goal_xpos, cur_mission_.goal_ypos, x, y );
		int state = -1;
			...
			if ( v <= 0.1) {
				if (!is_time_init_) {
					...
					if (!is_initialized_) {
						...
						ROS_INFO("GLOBAL PATH TO DROPOFF PUBLISHED");
					}
				}
				else {
					double duration = (ros::Time::now() - time_).toSec();
					if (duration > time_th_) {
						ROS_INFO("START CURRENT MISSION");
						is_reach_ = false;
						is_start_ = true;
						is_time_init_ = false;
						is_initialized_ = false;
