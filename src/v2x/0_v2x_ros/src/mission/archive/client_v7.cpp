...
private:
	...
	std::vector<int> yaml_mission_list;
	int key;
...
public:	
	...
	void initParams(void) {
		...
		private_nh_.getParam("/yaml_mission_list",yaml_mission_list);

		sock_, size_, is_connect_ = -1;
		key = 0;

		// std::cout<<" "<<obu_ip_<<" "<<obu_port_<<" "<<buff_size_<<" "<<rate_<<" "<<is_v2x_on_<<" "<<is_rehersal_<<" "<<use_custom_mission_<<" "<<dist_th_<<" "<<yaml_mission_list.size()<<" "<<std::endl;

		ROS_INFO("THIS IS YAML THE MISSION LIST");
		for (int i=0; i < yaml_mission_list.size(); i++){
			std::cout<<"mission "<<i+1<<" : "<<yaml_mission_list[i]<<std::endl;
		}		
	...
	void checkResponse(std::string packet) {
		...
		ROS_INFO("Packet Type is: |0x%2x| ",packet_type);
			...
			if (packet_type == 0x133F) {
				...
				// STEP 0: PRINT MISSIONS ONCE & PUTTING MISSIONS IN A BUFFER
				print_call(payload); 
				...
				if (confirmResult == 0x00){ 			// 성공
					...
					mission_state = listening;
					key++;
					if (key == yaml_mission_list.size()){ mission_state = no_more_missions;}
	...
	int EndPointArriveMessage(int mission_id, double latitude, double longitude, unsigned char speed){ //목적지 도착 완료 메시지End-Point Arrive Message, 프로토콜 2.3.2.6임
		...
		ROS_INFO("Sentisize is %d",sentisize);
		ROS_INFO("drop off packet size (should be 29) is %d",(int)sizeof(dropoff_packet));
	...
	void sendCurrentLocation(double * a_latitude, double * a_longitude, float * a_elevation, unsigned short * a_heading, unsigned char * a_speed) {
		...
		while (ros::ok()){
			...
			ROS_INFO("Sentisize is %d",sentisize);
			ROS_INFO("location packet size (should be 34) is %d",(int)sizeof(location_packet));
		...
		if (is_v2x_on_ == true){
			switch (mission_state){
				...
				case choosing_mission:{
					...
					// selected_mission = 50; //mission number to put
					selected_mission = yaml_mission_list[key];
					
					CallRequestMessage(selected_mission);
					// PrioritizeMission();
					// mission_state = going_to_start_pos;
					ROS_INFO("CHECK THE START AND GOAL OF CURRENT MISSION ON RVIZ");
					break;
				}
				case start_mission:{
