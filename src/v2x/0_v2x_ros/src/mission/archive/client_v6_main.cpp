...
public:
		...
		private_nh_("~")
	{
		...
		// PUBLISHERS
		pickup_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("pickup_position", 1, true);
		dropoff_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("dropoff_position", 1, true);
		target_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("move_base_simple/goal", 1, true);
		choose_mission_pub_ = nh_.advertise<mission_msgs::mission>("mission_list", 1, true);
		currpose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("initialpose", 1, true);
		...
		//................................
		temp_mission_lists.push_back(33);
		temp_mission_lists.push_back(34);
		temp_mission_lists.push_back(33);
		//................................
		...
		// pos_alert_timer_ = nh_.createTimer(ros::Duration(0.1), &Call_List_Client::SendEvery100ms, this);


		ROS_ERROR("STARTING SEND CURRENT LOCATION THREAD!");

		if ((is_v2x_on_ == true) && (is_connect_ != -1)){
			// publish_thread_ = std::thread(&DynamicPlanning::startThread, this);

			// th1 = std::thread(&Call_List_Client::sendCurrentLocation, this, &curr_pos_lat, &curr_pos_long, &curr_pos_z, &send_heading, &send_speed);
			// double a_latitude = 35.880143;
			// double a_longitude = 128.626841;
			// float a_elevation = 100;
			// std::thread th1(&Call_List_Client::sendCurrentLocation, this, &a_latitude, &a_longitude, &a_elevation, &send_heading, &send_speed);
			// th1.detach();
	...
	void receive_message(){
		...
		if ((size_ = recv(sock_, raw_data, buff_size_, MSG_NOSIGNAL)) >= 9) {
			...
			// ROS_ERROR("TIME taken to recv() is : %.4f",tt);
	...
	void checkResponse(std::string packet) {
		while (packet.size() != 0) {
			...
			if (is_during_mission_ == true){
				ROS_ERROR("is_during_mission state: %d\n\n", (int)is_during_mission_);
			}
			if (packet_type == 0x133A){
				ROS_ERROR("inside 0x133A\n\n");
					...
					std::ofstream myfile;
					myfile.open("/home/urlautocar/mission_2325.txt");
					myfile << output;
					myfile.close();
				sleep(2);
			}
			if (packet_type == 0x133C){
				ROS_ERROR("inside 0x133C\n\n");
					std::string output = string_to_hex(payload); //https://stackoverflow.com/questions/3381614/c-convert-string-to-hexadecimal-and-vice-versa
					std::cout<<output<<std::endl;
					std::ofstream myfile;
					myfile.open("/home/urlautocar/mission_2327.txt");
					myfile << output;
					myfile.close();
				sleep(2);
			}
			...
			if (is_during_mission_ == true ){
				if (packet_type == 0x133A || packet_type == 0x133C){
					ROS_ERROR("hello");
				} else{
					ROS_ERROR("got here");
					packet.clear();
			...
			if (packet_type == 0x133F) {
				...
				if (confirmResult == 0x00){ 			// 성공
					...
					mission_state = listening;
					key++;
					if (key == 3) {mission_state= no_more_missions;}
	...
	int print_call(const std::string &payload){
		...
		std::cout<<output<<std::endl;
		std::ofstream myfile;
		myfile.open("/home/urlautocar/mission_payload.txt");
		myfile << output;
		myfile.close();
		//
	...
	int CallRequestMessage(int mission_id){ //선점하고자하는 메시지를 보내는 것, 프로토콜 2.3.2.2임
		...
		int sentisize = -1;
		if (sentisize = send(sock_, call_packet, sizeof(call_packet), 0) > -1) {
	...
	int StartingPointArriveMessage(int mission_id, double latitude, double longitude, unsigned char speed){ //출발지 도착 완료 메시지Starting-Point Arrive Message, 프로토콜 2.3.2.4임
		...
		unsigned char crc_arr[2];
		memcpy(crc_arr, &check_result, 2);
		ROS_ERROR_STREAM("inside message:"<<mission_id);
		call_packet[9] = (unsigned char) mission_id;
		memcpy(&call_packet[10], lat_arr, sizeof(latitude));
		memcpy(&call_packet[18], long_arr, sizeof(longitude));
		memcpy(&call_packet[26], speed_arr, sizeof(speed));
		call_packet[27] = crc_arr[0];
		call_packet[28] = crc_arr[1];
		...
		// std::cout<<output<<std::endl;
		// std::ofstream myfile;
		// myfile.open("/home/urlautocar/send_2324.txt");
		// for (int i=0; i<29;i++){
		// 	myfile << call_packet[i];
		// }
		// myfile.close();

		// send(sock_, call_packet, sizeof(call_packet), 0);
		int sentisize = send(sock_, call_packet, sizeof(call_packet), 0);
		ROS_ERROR("sentisize is %d", (int)sentisize);
		...
		if ( sentisize > -1) {
			...
			ROS_ERROR("FAILED TO SEND ARRIVE AT PICKUP LOCATION PACKET - sentisize is: %d",(int)sentisize);
			return false;
		}
	}
	...
	int EndPointArriveMessage(int mission_id, double latitude, double longitude, unsigned char speed){ //목적지 도착 완료 메시지End-Point Arrive Message, 프로토콜 2.3.2.6임
		...
		int sentisize = send(sock_, call_packet, sizeof(call_packet), 0);
		ROS_ERROR("sentisize is %d", sentisize);

		if ( sentisize > -1) {
			ROS_INFO("SEND ARRIVE AT DROPOFF LOCATION PACKET SUCCESSFULLY");
			sequence_++;
			mission_state = listening;
			return true;
		}
		else {
			ROS_ERROR("FAILED TO SEND ARRIVE AT DROPOFF LOCATION PACKET, sentisize: %d", (int)sentisize);
			return false;
		}
	}
	...
	// void SendEvery100ms(const ros::TimerEvent& event){
	// 	ROS_ERROR("--timer callback!--");
	// 	if (is_rehersal_ == true){
	// 		sendCurrentLocation(curr_pos_lat, curr_pos_long, curr_pos_z, send_heading, send_speed, "rehersal");
	// 	} else if (is_rehersal_ == false){
	// 		sendCurrentLocation(curr_pos_lat, curr_pos_long, curr_pos_z, send_heading, send_speed, "notrehersal");
	// 	} else {
		...
		if (is_v2x_on_ == true){
			switch (mission_state){
				case listening:{
					...
					// ROS_INFO("TIME TAKEN TO RECEIVE MESSAGE IS : %.5f",t);
					break;
				}
...
//--------------------------------------------------------------------------------
					// selected_mission = 17; //mission number to put

					selected_mission = temp_mission_lists[key];
					ROS_ERROR_STREAM("selected mission isisisisi "<<selected_mission);
//--------------------------------------------------------------------------------
				...
				case pickingup:{
					...
					count5secs();
					ROS_ERROR_STREAM("selected mission is: "<<selected_mission);
	...
	std::vector<int> PrioritizeMission(void) {
		...
		if (is_rehersal_ == true){
			std::sort(mission_buf_.begin(), mission_buf_.end(), less_than_key());
			for (int i = 0; i < 60; i++){
				mission_order.push_back(mission_buf_[i].id);
			}
		} else {
			...
			mission_order.push_back(60); //just to make this function fit the description of returning type
		}
	...
	bool checkArrival(const std::string pickup_or_dropoff){ //pickup_or_dropoff = "pickup" OR "dropoff"
		...
		// ROS_INFO("APPROACHING %s POINT, %f METERS(?) LEFT| average speed is %f",pickup_or_dropoff.c_str(), dist/2, v);
