...
public:
	...
	void initRedlights(void){
		...
		// }
		// object_marker.publish(check_redlight_buf);
		// sleep(100);
		// ////////////// /////// /////// /////// ////////////// 
	...
	int run(){
		...
		if ((got_message_ == true)&& (msgFrame1 != nullptr)){
			...
			for (int j = 0; j < (int)iti->states.list.count; j++){
				...
				if (itk->eventState == 0 || itk->eventState == 5 || itk->eventState == 6 ){
			...
			ROS_ERROR("run() next time when spat is correctly recieved");
			return -1;
		}
	...
	void print_structure(struct TrafficSignal &intersection){
		std::cout<<"id: "<<intersection.intersection_id;
		for (int i=0; i<intersection.signalgroup_buf.size();i++){
			//ㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁ
			if (intersection.intersection_id != 10010) continue;
			//ㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁ
