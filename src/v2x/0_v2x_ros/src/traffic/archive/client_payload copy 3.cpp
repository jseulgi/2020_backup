				...
				for (int i = 0; auto& iti = spattmp.intersections.list.array[i]; ++i) {
					...
					for (int j = 0; auto& itj = iti->states.list.array[j]; j++){
						...
						traffic_signal[m][itj->signalGroup][1] = (double)itk->timing->minEndTime/10;
							...
							std::cout<< "signal group "<<itj->signalGroup<<"\'s event state is : " << traffic_signal[m][itj->signalGroup][0] << ", which means " <<std::endl;
...
int main(int argc, char** argv) {
	...
	// if testing with known SPat & MAP payload data
	vc.load_datasamples();
	...
	while(ros::ok()){
		...
		ROS_INFO("time for run is : %f and %f sec",mid_time-start_time,end_time-mid_time);
		r.sleep();
	}
