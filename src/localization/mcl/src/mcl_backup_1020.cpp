...
bool planar_cmp(pcl::PointXYZI a, pcl::PointXYZI b){
    return a.z > b.z;
}
...
mcl::mcl()
{
    ...
    static ros::Subscriber planar_sub = m_nh.subscribe<jslocalization::planars>("/jslocalization/feature/plane_obj", 10, &mcl::planar_callback, this);
    ...
    m_pub_map_planar = m_nh.advertise<sensor_msgs::PointCloud2>("/jslocalization/mcl/map_scan",1);
    ...
    planar_points_map.reset(new pcl::PointCloud<pcl::PointXYZ>());
...
void mcl::extract_closest_planar(jslocalization::planars planars, jslocalization::planars &out_planars)
{   
    pcl::PointCloud<pcl::PointXYZI> pc_scan;

    if(planars.planar.size()==0)
        return;

    for(size_t i=0;i<planars.planar.size();i++)
    {
        float a = planars.planar[i].slope;
        float b = planars.planar[i].start.y - a*planars.planar[i].start.x;
        float dist2origin = abs(b)/sqrt(pow(a,2)+1);

        pcl::PointXYZI temp;
        temp.x = planars.planar[i].index;
        temp.y = 0;
        temp.z = planars.planar[i].length;//dist2origin;
        pc_scan.points.push_back(temp);
        std::cout<<"\033[1;34m[MCL]\033[0m ["<<i<<"] dist: "<<dist2origin<<" length: "<<planars.planar[i].length<<std::endl;
    }
    ...
    for(size_t i =0; i<planar_max; i++){
        ...
        temp_planar = planars.planar[index];
    ...
    std::cout<<"\033[1;34m[MCL]\033[0m Total planar #: "<<planars.planar.size()<<" selcted pole #: "<<planar_max<<std::endl;
...
void mcl::data_association(pcl::PointCloud<pcl::PointXYZ>::Ptr map_cloud, pcl::PointXYZ observation, pcl::PointXYZ &closest_pole, int &closest_pole_id, float &min_dist)
{
...
void mcl::data_association(pcl::PointCloud<pcl::PointXYZI>::Ptr map_cloud, pcl::PointXYZI observation, pcl::PointXYZI &closest_planar, int &closest_planar_id, float &min_dist)
{
...
float mcl::calcWeight(float delta_ang, float covariance)
{
    ...
    for(size_t i=0;i<m_particles.size();i++)
    {
        ...
        else
        {
            ...
            {   
                ...
                data_association(pole_points_map, pole_transformed->points[j], closest_point, closest_id, min_dist);
                ...
                {   
                    weight += calcWeight(min_dist, poles_map.pole[closest_id].covariance);
                }
            }
...
//         Eigen::VectorXf pose_xyzrpy = cvt::eigen2xyzrpy(m_particles.at(i).pose);
//         max_particle.score += m_particles.at(i).score/6;
//         sum_x += pose_xyzrpy(0)/6;
//         sum_y += pose_xyzrpy(1)/6;
//         sum_theta += pose_xyzrpy(5)/6;
