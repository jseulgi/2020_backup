        ...
        if( abs(cluster_max.z - cluster_min.z) > po_height)
        {
            *pc_cluster2 += *clusters[i];
            ...
            for(int j = 0; j < num_slice; j++)
            {
                ...
                if(pc_height_slice->points.size()==0)
                    continue;

                pcl::PointXYZI slice_min, slice_max;
                pcl::getMinMax3D(*pc_height_slice, slice_min, slice_max);

                float cluster_radius = sqrt(pow(slice_min.x-slice_max.x,2)+pow(slice_min.y-slice_max.y,2))/2;

                // std::cout<<"cluster_radius : "<<cluster_radius<<std::endl;

                if(cluster_radius < po_radius)
                {
        ...
        if(trunk_max.z - trunk_min.z > po_trunk_height){
            ...
            *pc_cluster5 += *cluster_new;
            ...
            if(abs(trunk_min.z - incloud_min.z) < po_height_min)
            {
                ...
                pole_centroid.x = (trunk_min.x + trunk_max.x)/2;
                pole_centroid.y = (trunk_min.y + trunk_max.y)/2;
                pole_centroid.z = (trunk_min.z + trunk_max.z)/2;

                pcl::PointXYZI pole_center;
                pole_center.x = pole_centroid.x;
                pole_center.y = pole_centroid.y;
                pole_center.z = 0;

                float plane2pole=999;
                if(pc_plane->points.size()>0){
                    for(int j=0; j<pc_plane->points.size(); j++){
                        pc_plane->points[j].z = 0;
                    }

                    nearest_search(pc_plane, pole_center, plane2pole);
                    // std::cout<<"plane2pole: "<<plane2pole<<std::endl;
                }
...
void feature::nearest_search(pcl::PointCloud<pcl::PointXYZI>::Ptr target, pcl::PointXYZI src, float &min_dist)
{
    ...
    if ( kdtree.nearestKSearch (src, 1, pointIdxNKNSearch, pointNKNSquaredDistance) > 0 )
    {   
