...
from QtWidgets import QAction, QMenu, QWidget, QMessageBox, QSizePolicy, QLabel, QComboBox
...
class StatusLightWidget(QWidget):
    _FONT_COLOR = QColor(0,0,0,255)
    _UNKNOWN_COLOR = QColor("#dddddd")
    _SUCCESS_COLOR = QColor(92,184,92,255)
    ...
    def paintEvent(self, event):
        with self.lock:
            ...
            qp = QPainter()
            qp.begin(self)
            radius = 10#min(rect.width(), rect.height())/4
            qp.setFont(QFont('Helvetica', 15))
            qp.setPen(QPen(QBrush(color), 10))
            qp.setBrush(color)
            qp.drawEllipse((rect.width() - radius) / 4, (rect.height() - radius) / 2,
                           radius, radius)

            qp.setPen(QPen(QBrush(self._FONT_COLOR), 10))
            if self._active_topic is not None and self._active_topic.replace("/","") is not None:
                qp.drawText((rect.width() - radius) / 4 + 3*radius, (rect.height() - radius) / 2 + radius, (self._active_topic.replace("/","")).replace("_"," "))
            else:
                qp.drawText((rect.width() - radius) / 4 + 3*radius, (rect.height() - radius) / 2 + radius, "")
            qp.end()
            return
