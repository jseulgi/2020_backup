...
#ifdef PCAN_SJA1000_STATS
	memset(dev_stats + dev->can_idx, 0, sizeof(struct pcandev_stats));
#endif
...
#ifdef PCAN_SJA1000_STATS
	sja1000_print_stats(dev->adapter->name, dev->can_idx);
#endif
}
...
#ifdef PCAN_SJA1000_STATS
	if (dev_stats[dev->can_idx].max_rx_msgs < msgs)
		dev_stats[dev->can_idx].max_rx_msgs = msgs;
#endif
	return result;
}
...
#ifdef PCAN_SJA1000_STATS
	dev_stats[dev->can_idx].write_frm_count++;
#endif
...
#ifdef PCAN_SJA1000_STATS
		dev_stats[dev->can_idx].write_count++;
#endif
	}
...
irqreturn_t __pcan_sja1000_irqhandler(struct pcandev *dev)
{
...
#endif
#ifdef PCAN_SJA1000_STATS
			dev_stats[dev->can_idx].int_tx_count++;
#endif
#if 0
			chipstatus = dev->readreg(dev, SJA1000_SR);
...
#ifdef PCAN_SJA1000_STATS
			dev_stats[dev->can_idx].int_rx_count++;
#endif
#if 0
			chipstatus = dev->readreg(dev, SJA1000_SR);
			if (!(chipstatus & SJA1000_SR_RBS)) {
...
#ifdef PCAN_SJA1000_STATS
			dev_stats[dev->can_idx].int_ovr_count++;
#endif
#if 0
			chipstatus = dev->readreg(dev, SJA1000_SR);
...
#ifdef PCAN_SJA1000_STATS
			dev_stats[dev->can_idx].int_err_count++;
#endif
...
#ifdef PCAN_SJA1000_STATS
	if (dev_stats[dev->can_idx].max_int < j)
		dev_stats[dev->can_idx].max_int = j;

	if (ret == PCAN_IRQ_HANDLED)
		dev_stats[dev->can_idx].int_count++;
#endif

	if (wwakeup) {
#ifdef PCAN_SJA1000_STATS
		dev_stats[dev->can_idx].wakup_w_count++;
#endif
	...
	if (rwakeup) {
#ifdef PCAN_SJA1000_STATS
		dev_stats[dev->can_idx].wakup_r_count++;
#endif
